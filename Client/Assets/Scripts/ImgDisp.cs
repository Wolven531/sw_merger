...
namespace awillInc {
    public class ImgDisp : MonoBehaviour
    {
        ...
        void loadSpriteImage(string targetURL, GameObject g, string fileName = "")
        {
            if (isLoading)
            {
            ...
            StartCoroutine(loadSpriteImageHelper(targetURL, g, fileName));
        }
        ...
        /// <param name="target"></param>
        /// <returns></returns>
        IEnumerator loadSpriteImageHelper(string targetURL, GameObject target, string fileName = "")
        {
            isLoading = true;
            if(string.IsNullOrEmpty(fileName)){
                fileName = System.IO.Path.GetFileName(targetURL);
            }
